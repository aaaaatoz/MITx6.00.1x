...
class WordTrigger(Trigger):
    ...
    def isWordIn(self,testingString):
        evaluteString = testingString.lower()
        for char in string.punctuation:
            evaluteString = evaluteString.replace(char,' ')
        stringlist = evaluteString.split()
        if self.word in stringlist:
            return True
        else:
            return False
...
class PhraseTrigger(Trigger):
    def __init__(self,phrase):
        self.phrase = phrase
    def evaluate(self, story):
        return  self.phrase in story.getGuid() or self.phrase in story.getTitle() or self.phrase in story.getSubject() or self.phrase in story.getSummary() 
...
def filterStories(stories, triggerlist):
    ...
    for story in stories:
        for trigger in triggerlist:
            if trigger.evaluate(story) and story not in returnStories:
                returnStories.append(story)
                break
...
def makeTrigger(triggerMap, triggerType, params, name):
    ...
    """
    # TODO: Problem 11
    # TriggerType can be 'subject','title','phrase','SUMMARY','AND,NOT,OR','PHRASE'
